    ...
    try {
      ...
      const res = await fetch("/api/writer", {
        ...
        body: JSON.stringify({
          ...
          sceneDescription,
          expectedWordCount: getTargetLength(),
          styleRequirement,
        }),
      });
              ...
              <div className="flex flex-col gap-6 p-5">
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <Tabs value={vibe} onValueChange={(v) => setVibe(v as SliceParams["vibe"])}>
                    ...
                    <StyleTab value="冷艳华美">偏意象堆叠与金属质感，句子略长但不黏连。</StyleTab>
                    <StyleTab value="温润烟火">偏生活肌理与细碎声响，隐喻落在日常器物上。</StyleTab>
                    <StyleTab value="疏离克制">短句与停顿偏多，不写满、不写透。</StyleTab>
                    <StyleTab value="浪漫诗性">允许轻微的象征与复沓，仍以场景为底板。</StyleTab>
                  </Tabs>
                </div>
...
function StyleTab({ value, children }: { value: string; children: ReactNode }) {
  return (
    <TabsContent
      value={value}
