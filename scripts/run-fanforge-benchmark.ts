...
function renderMarkdown(baseUrl: string, caseSource: string, results: BenchmarkResult[]) {
  ...
  const lines = [
    ...
    "### Phase 2 MVP",
    "",
    "- Add 7 MVP synthetic cases in `docs/benchmark/fanforge-cases.json`: 2 Canon, 2 Persona, 2 Emotion Slice, and 1 Multi-Agent case.",
    "- Validate that the runner can read cases, call existing APIs, or mark unavailable/authenticated paths as `skipped`.",
    "- Add rule-based evaluators for Canon conflicts, OOC risk, relationship stage jumps, direct confession, over-explanation, and forbidden-line violations.",
    "- Generate `fanforge-cases.md` and this `fanforge-analysis.md` report with stable metric structure.",
    "- Treat Phase 2 as runner and metric validation, not final resume data.",
    "",
    "### Phase 3 Final Benchmark",
    "",
    "- Expand to 30 high-quality synthetic cases, not 40/60 volume testing.",
    "- Final split: 8 Canon Consistency, 8 Persona Timeline, 8 Emotion Slice, and 6 Multi-Agent Revision cases.",
    "- Use the 30-case run for portfolio-facing metrics such as Canon evidence hit rate, OOC risk reduction, Persona stage constraint hits, relationship violation reduction, and Multi-Agent issue reduction.",
  ];
